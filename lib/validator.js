...
var moment = require('happy-moment');
    ...
    for (var parameterName in rules) {
      ...
      if (rule.validator) {
        rule.validator.validate(
          parameterValue,
          function (message, childName, childValue) {
            var name;
            if (childName) {
              if (childName[0] === '[') {
            ...
            result |= onError(
              message,
              name,
              childValue !== undefined ? childValue : parameterValue,
            );
          },
        );
      }
  ...
  function validate(value, onError) {
    ...
    if (options) {
      if (options.regex && !options.regex.test(value)) {
        return onError(
          options.message || 'Invalid value. Value must match required pattern.',
        );
      }
    }
    return null;
...
function isNumber(options) {
  ...
  function validate(value, onError) {
    ...
    var acceptType =
      util.isNumber(value) ||
      (options['allowString'] && util.isString(value) && !isNaN(value));
  ...
  function validate(value, onError) {
    ...
    if (options.format) {
      if (!moment(value, options.format, true).isValid()) {
        return onError(
          options.message ||
            'Invalid value. Format is not valid. Expected ' + options.format,
        );
      }
    } else {
  ...
  function validate(value, onError) {
    ...
    if (options) {
      if (options.min && value.length < options.min) {
        return onError(
          'Count must be greater than or equal to ' + options.min + '.',
          'length',
          value.length,
        );
      }
      if (options.max && value.length > options.max) {
        return onError(
          'Count must be less than or equal to ' + options.max + '.',
          'length',
          value.length,
        );
      }
    ...
    if (validator) {
      for (var j in value) {
        validator.validate(value[j], function (message, childName, childValue) {
          result |= onError(
            message,
            childName ? '[' + j + '].' + childName : '[' + j + ']',
            childValue !== undefined ? childValue : value[j],
          );
        });
      }
    }
